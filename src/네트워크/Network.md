...
|-------|---------------|------------------------------|
| 주소 길이 | 32비트          | 128비트                        |
| 주소 유형 | 10.80.162.73  | fe80::3854:9d97:c368:c246%16 |
| 주소 개수 | 약 43억개        | 약 340간                       |
...
- 클라이언트 측의 요청을 전달하기 위해 서버가 이해할 수 있는 데이터로 변환하고 전송계층으로 전달하는 역할을 한다
...
- 클라이언트에서 `HTTP 요청(Request)`를 보내고 서버에서 `HTTP 응답(Response)`를 반환한다
- `Keepalive` : HTTP/1.1 버전에서 데이터 교환이 마칠 때까지 연결을 유지하고 끝나면 연결을 끊는 구조이다
- HTTP/2 버전에서는 요청을 보낸 순서대로 응답을 반환하지 않아도 된다


### DNS 서버의 구조

###### DNS
- URL 을 IP 주소로 변환하는 서비스이다
- `이름 해석` : 웹사이트에 접속하면 웹사이트의 서버 IP 주소를 알려주는 것
- `도메인 이름` : 컴퓨터나 네트워크를 식별하기 위해 붙여진 이름
- `호스트 이름` : 도메인 이름 앞에 있는 www 를 말한다
- 요청받은 DNS 서버가 해당 도메인 이름의 IP 주소를 모르면 다른 DNS 서버에 요청한다
- DNS 서버는 전 세계에 흩어져 있고 연계하면서 동작한다


### 메일 서버의 구조
- `SMTP` : 메일을 보내는데 사용되는 프로토콜, 25번 포트를 사용한다
- `POP3` : 메일을 받는데 사용되는 프로토콜, 110번 포트를 사용한다
- `메일 박스` : 메일을 보관해주는 곳
...
###### 네트워크의 구성
![title](./image/네트워크의%20구성.png)

- 웹에 접속할 때는 OSI 모델의 상위 계층부터 캡슐화가 이루어진다
- 웹 서버에 요청을 보낼 때는 HTTP 를 사용한다
- `세그먼트` : 전송 계층에서 TCP 헤더가 추가된 데이터
- `IP 패킷` : 네트워크 계층에서 IP 헤더가 추가된 데이터
- `이더넷 프레임` : 데이터 링크 계층에서 이더넷 헤더와 트레일러가 추가된 데이터
...
- 라우터 내에 대응되는 OSI 모델의 계층은 `물리 계층`과 `데이터 링크 계층`과 `네트워크 계층`이다


### 웹 서버에서의 데이터 전달과 처리

- 웹 서버 내에 해당하는 OSI 모델의 계층은 `물리 계층`, `데이터 링크 계층`. `네트워크 계층`, `전송 계층`, `응용 계층`이다
...
###### 무선 랜
- 랜 케이블이 필요하지 않는다
- `무선 액세스 포인트`와 `무선 클라이언트`로 구성된다

![title](./image/무선%20액세스포인트와%20무선%20클라이언트.png)
...
- `IEEE802.11`은 IEEE 에서 승인한 무선 랜의 표준화 기술이다

| 무선 랜 규격      | 통신속도(최대) | 주파수 대역    | 특징                                  |
|--------------|----------|-----------|-------------------------------------|
| IEEE802.11ad | 6.7Gbps  | 60GHz     | 초고속 통신이다                            |
| IEEE802.11ac | 6.9Gbps  | 5GHz      | 장애물이 많아도 고속통신을 할 수 있고 전파 간섭을 적게 받는다 |
...
###### SSID
- 액세스 포인트의 고유이름
- 무선 액세스 포인트와 무선 클라이언트를 연결하려면 혼선을 피하기 위해 사용한다
- `네트워크 이름`, `인증`, `암호화`, `암호화 키`를 설정해야 한다
- 무선 액세스 포인트는 `비컨`이라고 하는 자기를 알리는 신호를 네트워크에 있는 모든 기기에 주기적으로 전송한다

###### 채널
- 여러기기를 동시에 연결할 수 있도록 주파수 대역을 분할할 때의 주파수 대역